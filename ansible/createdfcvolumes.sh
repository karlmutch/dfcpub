...
for i in `cat inventory/targets.txt`; do
        ...
        for j in $(seq 1 $1); do
                ...
                echo aws ec2 detach-volume --volume-id ${volumeId} --force | tee -a removevolumes.sh
                echo aws ec2 delete-volume --volume-id ${volumeId} | tee -a removevolumes.sh
        done
done

chmod 744 attachvolumes.sh
chmod 744 removevolumes.sh
