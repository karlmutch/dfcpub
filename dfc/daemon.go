...
func dfcinit() {
	...
	if clivars.role == xproxy {
		...
		if err := iostat.CheckIostatVersion(); err != nil {
			glog.Exit(err)
		}
		...
		iostat := iostat.NewRunner(fs.Mountpaths, &ctx.config.Periodic.StatsTime)
...
func getiostatrunner() *iostat.Runner {
	r := ctx.rg.runmap[xiostat]
	rr, ok := r.(*iostat.Runner)
	cmn.Assert(ok)
	return rr
}
